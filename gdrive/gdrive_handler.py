...
class GspreadHandler:
    ...
    def get_row_by_timestamp(self, sheet_name, worksheet_name, timestamp):
        ...
        """
        print(f"timestamp input: <{str(timestamp)}>")
        # enclose str(timestamp) with " so that it becomes from 1719205230160872300 to "1719205230160872300"
        
        str_timestamp = str(timestamp)
        
        worksheet = self.get_sheet(sheet_name, worksheet_name)
        df = pd.DataFrame(worksheet.get_all_records())
        test = df["itinerary_id"].iloc[-1]
        ...
        if 'itinerary_id' in df.columns:
            ...
            matching_row = df.loc[df['itinerary_id'].str.lower() == str_timestamp.lower()]
            print(matching_row)
